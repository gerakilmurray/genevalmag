...
% 
% NOTA: Se podría ver como: proviene de una partición posible que no puede ser plausible.
% \end{definition}
% 
% \begin{definition}
...
% 
% NOTA: se lo podría ver como: un plan es plausible si proviene de la union de particiones plausibles.
% \end{definition}
...
% El algoritmo \ref{alg:planeval} genera la mínima cantidad de planes o dicho en otras palabras, genera solamente planes plausibles.

%\underline{\emph{Demostración:}}

% SACAR ESTE Y EL ANTERIOR DESDE EL PAPER DE WUU YANG DE CLASIFICACIÓN.
%\subsection{Teorema2}
...
%El algoritmo \ref{alg:planeval} genera planes para todas las producciones alcanzables de la gramática y tiene en cuenta todos los ordenes $\omega$ necesarios.
...
De esta forma, cuando se detecta un \textbtt{visit}, se debe localizar a que hijo tiene que recursar en base al plan actual, el cual tiene un contexto establecido. Además se debe determinar el plan proyectado que actuará de orden de e\-va\-lua\-ción impuesto en la recursión. Al volver se debe actualizar todas las instancias de atributos que fueron computadas en el contexto inferior, para evitar futuras visitas innecesarias.
...
El algoritmo teórico escogido para implementación de los evaluadores generados, es el presentado en el algoritmo \ref{alg:evalvisiter}.
...
Las dos etapas anteriores son realizadas dentro del algoritmo principal denominado \textbtt{eval}, mostrado en \ref{alg:evalattr}.
