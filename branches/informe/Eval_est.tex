...
De los puntos \ref{th:p1} y \ref{th:p2}, todos los ordenes de evaluación $\Psi$ computados por \textbtt{compute\_order} son compatibles con los ordenes parciales para $ADP (q | p_{1}, p_{2}, \dots, p_{k})$ $\cup$ $\omega$, por lo tanto, la evaluación es consistente para la producción $q$ cuando los atributos del símbolo \textbtt{head} de $q$ son evaluados en el orden $\omega$ y $p_{1}, p_{2}, \dots, p_{k}$ son producciones aplicadas a los símbolos no terminales del \textbtt{body} de $q$. Como el algoritmo es un método exhaustivo, se puede afirmar que todas las producciones alcanzables $q$ y todos los ordenes $\omega$ necesarios son examinados por el algoritmo.
