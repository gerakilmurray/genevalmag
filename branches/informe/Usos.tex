...
Para usar \maggen\ se debe invocar la herramienta mediante la línea de comando, es decir, por lo se es necesario contar con una terminal\footnote{En sistemas \textit{Windows} compatible: \textbf{Win+R} y luego \texttt{cmd} y en sistemas \textit{Unix}: \textbf{Alt+F2} y luego \texttt{xterm}} para su ejecución. Por cuestiones de simplicidad y flexibilidad de la herramienta, no se consideró el desarrollo de un \textit{front-end} para \maggen. Además, se debe tener en cuenta, que el usuario debería presentar mayor interés en el resultado producido por \maggen\ y no de una innecesaria interfaz gráfica, que entorpecería su comportamiento combinatorio con otros comandos.
...
% Comenzaremos con la sección de compilación e instalación de la herramienta.
...
Una vez descargado la última versión de \maggen, se dispondrá de un directorio llamado ``\texttt{\textbf{maggen/}}'', dentro del cual se alojan todos los archivos de la herramienta.

Para compilar e instalar \maggen, primero el usuario deberá posicionarse dentro del directorio ``\texttt{\textbf{maggen/}}'', mediante las funciones de la terminal, y luego deberá ejecutar los siguientes comandos:
...
\subsection{Directorio \textbf{\texttt{maggen}}}
...
\begin{description}
\item [-f  file] Definir el archivo de entrada de \maggen\ como \texttt{file}. Si se omite esta opción \maggen\ espera la entrada por la entrada estándar (\texttt{cin}) hasta leer el carácter \texttt{EOF} (end of file)\footnote{En sistemas \textit{Unix} el carácter \texttt{EOF} puede ser producido con \textbf{Ctrl+D} dentro de una consola de comandos.}.
...
\item [-fo folder] Define \texttt{folder} como el directorio de salida para \maggen. Si se omite esta opción, \maggen\ usa el folder por defecto ``\textit{./out\_maggen/}''.

\item [-o  name] Define a \texttt{name} como el nombre de la clase y del archivo generado por \maggen. Si se omite esta opción, \maggen\ usa por defecto el nombre ``\textbf{mag\_eval}''.
...
\begin{itemize}
\item Si se invoca:
\begin{center} {\small\texttt{./maggen -h}}\end{center} se obtiene el siguiente mensaje de ayuda:
...
\item Si se invoca a \maggen\ sin la opción \texttt{-f} (independientemente que se usen, o no, las demás opciones) se espera la entrada por entrada estándar. Esto permite poder utilizar a \maggen\ en conjunto con otros comandos de forma encadenada. Por ejemplo:\\

\texttt{cat file | maggen -fo ./out/}.\\

Esto muestra a \maggen\ en conjunto con \texttt{cat} usando una tubería (\texttt{pipe}, en ambientes \textit{Unix} mediante el operador ``\texttt{|}'').
...
\begin{center}
{\scriptsize\texttt{./maggen -fo ./Out\_wuu\_yang -o maggen -f ./examples/ag\_wuu\_yang/ag\_wuu\_yang.input}}\end{center}
...
Dicha información es almacenada en la ruta \texttt{./Out\_wuu\_yang}, dentro de la cual se encuentran:
...
\vspace{0.4cm}
\lstinputlisting[basicstyle=\tiny, numbers=left]{input_file_code/ag_wuu_yang.cpp}
\vspace{0.4cm}
