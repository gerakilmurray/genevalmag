...
\begin{lstlisting}[escapeinside=<>, frame=shadowbox, rulesepcolor=\color{azul},language=inform, linewidth=10cm]
...
\begin{lstlisting}[escapeinside=<>, frame=shadowbox, rulesepcolor=\color{azul}, language=inform ]
...
\begin{lstlisting}[frame=shadowbox, rulesepcolor=\color{azul}, language=inform]
decl_rules = (d_rule)+ 
...
Los dos módulos analizados arriba, generados por \maggen, se apoyan sobre dos módulos mas, que son anexados de manera estática por la herramienta, ellos son: \texttt{Node.hpp} y \texttt{Plan.hpp}. Los mismos contienen definiciones de ``tipos'' y ``metodos'' necesarios por el el evaluador generado.
