...
El proceso de construir un plan es complejo, la heurística seguida respeta el algoritmo para computar planes presentado por Wuu Yang.
...
\begin{itemize}
\item \textbf{WL} \flecha \texttt{vector <Item\_work> work\_list;}
\item {\LARGE\textbf{$\Delta$}} \flecha \texttt{vector <Item\_work> defined\_item\_work;}
\item \textbf{ComputeOrder} \flecha \texttt{compute\_order}
\item \textbf{Project} \flecha \texttt{purge\_plan\_with}
\item {\LARGE\textbf{$\Theta$}} \flecha \texttt{map <Key\_plan, unsigned short> eval\_plans;}
\item {\LARGE\textbf{$\gamma$}} \flecha \texttt{map <Key\_plan\_project, unsigned short> plans\_project;}
\end{itemize}
