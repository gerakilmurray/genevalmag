...
\begin{lstlisting}[float=!ht, backgroundcolor=\color{white}, columns=fullflexible, linewidth=7cm]
E := T (op_infix E)*
...
\begin{lstlisting}[float=!ht, language=C++, basicstyle=\scriptsize, columns=fullflexible, linewidth=13cm]
...
La heurística utilizada, consiste en recorrer el \texttt{map} de símbolos no terminales y verificar que existe al menos una regla que lo tenga como parte izquierda.
...
\item Luego se inserta en la secuencia de visita local el \texttt{visit}.
...
\item Al terminar la recursión, la \texttt{gen\_visit\_seq} (invocación), se extraen los resultados útiles para este contexto.
...
Notar que recibe como parámetro una referencia a los ``\texttt{headers}'' que el usuario proporcionó en el momento de la invocación de \maggen \footnote{Ver sección \ref{sec:uso-maggen} para obtener detalles sobre parametros de invocación.}. Los mismos serán incluidos en el archivo \texttt{.cpp}.
...
\fbox{\texttt{\textbf{void traverse (struct Node *node, unsigned short order)}}}
...
Después se arma un \texttt{Key\_plan} con dicho índice y el parámetro \texttt{order}, que es el índice del plan proyectado que se corresponde con el orden de evaluación de sus ecuaciones, impuesto por su contexto superior.
...
\fbox{\texttt{\textbf{void eval\_visiter(struct Node *root)}}}
