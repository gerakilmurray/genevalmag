...
En el capítulo de implementación de \maggen\ (Ver cap. \ref{chap:implem}) se abordarán detalles sobre el funcionamiento de estas tres operaciones, tanto para el algoritmo de generación de secuencias, como así también para el evaluador generado. Uno de los principales cambios, con respecto a lo visto en los puntos de arriba, es que el índice $i$ es omitido (en \textbtt{visit} y \textbtt{leave}) debido a que cada nodo lleva un propio contador de invocaciones y mantiene los cambios estados en cada una de ellas.  
