...
\begin{itemize}
 ...
 \item Evaluador generado es estático.
\end{itemize}
}
...
\frame{
    ...
    \end{block}
    \begin{center}
\textbf{Al momento no existían herramientas basadas en esta familia de Gramáticas de Atributos}    \end{center} 
...
$$
se denota como $ \textbf{ADP} (q | p_{1}, p_{2}, \dots, p_{k}) $
\end{block}
$DCG_{X}(p)$ contiene las dependencias, entre las instancias de la gramática, para el símbolo \texttt{X}, acotando el análisis para la producción \textit{p} y los posibles contextos inferiores.
$$
...
\frame{
	...
	Un plan de evaluación esta dado por un orden en la computación de las ecuaciones que definen a cada instancia de la gramatica.
	\end{block}
}
