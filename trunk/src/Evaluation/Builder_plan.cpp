...
void Builder_plan::generates_topological_order(const Dp_graph &graph, Order_eval_eq &result_order, const Attr_grammar &grammar, const Context_rule &context_rule) const
{
	...
	for (MakeOrder::iterator i(make_order.begin()); i != make_order.end(); ++i)
	{
		...
		if (ins)
		{
			...
			if (ins->get_attr()->is_inherit() && ins->get_symb()->equals(*grammar.get_rule(context_rule.second[0]).get_left_symbol()))
			{
				vector<unsigned short> context_father;
				context_father.push_back(context_rule.first);
				index = (grammar.get_index_eq_with_context(ins, context_father));
			}
			else
			{
				index = (grammar.get_index_eq_with_context(ins, context_rule.second));
			}
			assert (index > 0);
...
void Builder_plan::print_all_plans(const Attr_grammar &grammar) const
{
	for(map < Key_plan, Order_eval_eq >::const_iterator it(eval_plans.begin()); it != eval_plans.end(); it++)
	{
		...
		const Rule *rule(&(grammar.get_rules().find(it->first.first.second[0])->second));
		name_graph.append(cleaning_tabs(rule->to_string_not_eqs()));

		if (it->first.first.second.size() == 1)
		{
		...
		else
		{
			...
			for(size_t i(1); i < it->first.first.second.size(); i++)
			{
				name_graph.append(" R");
				stringstream key_rule;
				key_rule << it->first.first.second[i];
				name_graph.append(key_rule.str());
				if(i < it->first.first.second.size() - 1)
				{
					name_graph.append(" ,");
				}
			}
		...
		for(size_t i(0); i < it->first.second.size(); i++)
		{
			...
			stringstream key_eq;
			key_eq << it->first.second[i];
			name_graph.append(key_eq.str());
			if(i < it->first.second.size() - 1)
			{
				name_graph.append(" ,");
			}
		}
...
void Builder_plan::generate_plans(const Attr_grammar &grammar, const Builder_graphs &build_graphs)
{
	...
	for(size_t i(0); i < initial_rules.size(); i++)
	{
		...
		context.first = 0; // rule ficticia; regla inicial no tiene padre.
		vector < unsigned short > invoque_context;
		invoque_context.push_back(initial_rules[i]);
		context.second = invoque_context;
		generates_topological_order(build_graphs.get_dcg_graph(initial_rules[i]), init_order, grammar, context);

		Key_work_list key;
		key.first = 0;//rule ficticia; regla inicial no tiene padre
		key.second = initial_rules[i];
		Item_work i_eval(key, init_order);
	...
	while(work_list.size() > 0)
	{
		...
		// A(q,w) is not marked;
		{
			...
			{
				if (adp->first[0] == i_work.first.second)
				{
					Context_rule context;
					context.first = i_work.first.first;
					context.second = adp->first;

					Order_eval_eq total_order(compute_order(adp->second, i_work.second, grammar, context));

					Key_plan key_plan;
					key_plan.first = context;
					key_plan.second = i_work.second;
					...
					for(size_t i(0); i < right_side.size(); i++)
					{
						...
						key_project.first = key_plan;
						key_project.second = right_side[i];
						...
						Key_work_list key;
						key.first = adp->first[0];
						key.second = adp->first[i+1];
						Item_work o_rule(key, proj_order);
	...
	cout << "PLANES GENERADOS EXITOSAMENTE" << endl;
