...
\begin{lstlisting}[float, language=C++, basicstyle=\scriptsize,numbers=left, columns=fullflexible,linewidth=8cm]
...
\begin{lstlisting}[float=t!, language=C++, basicstyle=\scriptsize, numbers=left, columns=fullflexible,linewidth=6cm]
my_grammar g;
...
\begin{lstlisting}[float=h!, language=C++, basicstyle=\scriptsize, numbers=left, columns=fullflexible,linewidth=11cm]
...
\begin{lstlisting}[language=C++, basicstyle=\scriptsize, numbers=left,columns=fullflexible,linewidth=9cm]
...
\begin{lstlisting}[language=C++, basicstyle=\scriptsize,numbers=left,columns=fullflexible,linewidth=5cm]
symbols <> st_sorts;
...
\begin{lstlisting}[language=C++, basicstyle=\scriptsize, numbers=left,numbers=left,columns=fullflexible,linewidth=13cm]
...
\begin{lstlisting}[language=C++, basicstyle=\scriptsize, numbers=left, numbers=left,columns=fullflexible,linewidth=9cm]
...
\begin{lstlisting}[float, language=C++, basicstyle=\scriptsize, numbers=left,columns=fullflexible,linewidth=13cm]
...
\begin{lstlisting}[language=C++, basicstyle=\scriptsize, numbers=left,columns=fullflexible,linewidth=7cm]
...
\begin{lstlisting}[language=C++, basicstyle=\scriptsize, numbers=left,linewidth=9cm]
...
\begin{lstlisting}[language=C++, basicstyle=\scriptsize, numbers=left,linewidth=10cm]
typedef char                           char_t;
...
El conjunto de estas secuencias, son los planes proyectados, es decir, las exigencias que se le imponen a una regla desde un contexto superior que la invoca a que se compute. Para el símbolo inicial, esta secuencia es obtenida como el órden topológico de su grafo DCG, ya que este contiene todas las dependencias entre los atributos de la regla, el cual si o si, será un órden de computación consistente para sus ecuaciones.
...
\begin{lstlisting}[language=C++, basicstyle=\scriptsize, numbers=left,linewidth=11.5cm]
...
\begin{lstlisting}[language=C++, basicstyle=\scriptsize, numbers=left, numbers=left,columns=fullflexible,linewidth=8.5cm]
...
\begin{lstlisting}[language=C++, basicstyle=\scriptsize, numbers=left, numbers=left,columns=fullflexible,linewidth=8cm]
...
\begin{lstlisting}[language=C++, basicstyle=\tiny, numbers=left, columns=fullflexible,columns=fullflexible,linewidth=14.5cm]
...
\item Enpareja la vieja secuencia, verificando posición a posición que sean iguales, pero si en la secuencia ya guardada existe un \texttt{leave}, es decir, un cero, vuelve a avanzar.
...
\underline{Por ejemplo:}\hspace*{1.7cm} \textbf{[4,-3,3,-1,0]} update con \textbf{[4,-3,3,-1,2,-2,1]}\\
\hspace*{4cm} da como resultado: \textbf{[4,-3,3,-1,0,2,-2,1]}.
...
Para obtener un diseño dentro de los estándares de C++, se crean dos archivos: una cabecera \texttt{.hpp} y un fuente \texttt{.cpp}.

Ambos archivos son creados con el nombre proporsionado por el usuario mediante la opción \texttt{maggen -o name} o sino con el nombre por defecto: \texttt{mag\_eval}.
...
\begin{lstlisting}[language=C++, basicstyle=\scriptsize, numbers=left, columns=fullflexible,columns=fullflexible,linewidth=10cm]
...
Notar que recibe como parámetro una referencia a los ``\texttt{headers}'' que el usuario proporsionó en el momento de la invocación de \maggen\footnotetext{Ver capítulo \ref{XXX} sobre Usos}. Los mismos serán incluídos en el archivo \texttt{.cpp}.
...
\begin{lstlisting}[language=C++, basicstyle=\scriptsize, numbers=left, columns=fullflexible,columns=fullflexible,linewidth=7cm]
...
Lo primero que el ``desarrollador'' se encuentra al abrir este archivo, es la implementación del constructor y del método \texttt{to\_string()} de cada uno de los símbolos no terminales. Por nuestro diseño, cada nodo del AST tiene un identificador de regla, que debe ser un \textbf{número positivo mayor que cero}.

A modo de ejemplo, observemos el código para el símbolo Y, del ejemplo de Wuu Yang en el apéndice \ref{XXX}.

\begin{lstlisting}[language=C++, basicstyle=\scriptsize, numbers=left, columns=fullflexible,columns=fullflexible,linewidth=9cm]
...
El algoritmo que se tuvo que implementar para poder evaluar un AST, es el propuesto por Wuu Yang en \ref{XXX}. El cual se compone de dos funciones:

\begin{items}
\item \texttt{traverse}:
\item \texttt{eval\_visiter}:
\end{items}
