...
\begin{lstlisting}[float, language=C++, basicstyle=\scriptsize,numbers=left, numbersep=5pt, numberstyle=\tiny]
...
\begingroup
\begin{lstlisting}[float=t!, language=C++, basicstyle=\scriptsize, numbers=left, numbersep=5pt, numberstyle=\tiny]
my_grammar g;
...
\begin{lstlisting}[float=h!, language=C++, basicstyle=\scriptsize, numbers=left, numbersep=5pt, numberstyle=\tiny]
...
Como ya me sencionó en el capítulo \ref{XXX}, varias son las condiciones que se deben chequear.
...
Planes de evaluación proyectados {\LARGE\textbf{$\gamma$}} & \texttt{map <Key\_plan\_project, unsigned short>\ plans\_project} \\
\hline
\end{tabular}

\vspace{0.3cm}

La función \texttt{compute\_order} debe analizar las .
