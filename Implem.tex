...
El usuario es el responsable de definir las ``\textttb{semantic actions}'' para lograr capturar los resultados intermedios, producidos durante la etapa de parsing y no sólo obtener un valor lógico, sobre si se pudo o no consumir toda la cadena de entrada.

Para comenzar a definir una gramática hay que crear una estructura que herede de la clase \textttb{grammar}. Dentro de ella se debe definir a su vez otra estructura templatizada denominada \textttb{definition}. En donde realmente estará la definición de la gramática.
...
\subsection{\textttb{``semantic domain''} en \spirit}

En esta sección de la especificación se debían aceptar tres tipos de elementos: \textttb{sorts}, \textttb{operators} y \textttb{functions}.

Los \textttb{sorts} serán los tipos que se podrán utilizar para definir los dominios e imágenes de los operadores y funciones.
...
\begin{description}
\item [\textttb{sorts}] Se exige que luego del identificador ``\textttb{sort}'' existe un espacio\footnote{\label{foot:espacio}Ver definición de \textttb{space\_p} en \ref{parsers}}. El nombre leído, se usará para crear un nuevo Sort dentro de la GA y también se insertará en la tabla de símbolos de sorts. Se acepta una lista de nombres de sorts para comodidad del usuario.
...
\item [\textttb{operators}] Para los operadores también se exige el espacio$^{\ref{foot:espacio}}$ luego de la cadena ``\textttb{op}''. Por definición se aceptan tres tipos: infijos, prefijos y posfijos, por lo que se hizo una regla para cada uno, solamente discriminado los dominios, ya que todos poseen una sóla imagen. Notar que los sorts permitidos, son sacados directamente de las tablas de símbolos destinadas para ese propósito.
...
\item [\textttb{functions}] Para las funciones se permiten como dominio listas de sorts, los cuales serán agregados incrementalmente a la función que se está declarando. La utilización de acciones semánticas facilitan la modularización. La declaración comienza con la cadena ``\textttb{function}'' seguida de una espacio$^{\ref{foot:espacio}}$.
...
\subsection{\textttb{``attributes''} en \spirit }
...
La vinculación de los atributos con sus respectivos dueños, se realizará más adelante, debido a que existe la posibilidad de declarar un atributo para todo símbolo, mediante la sentencia \textttb{all}, antes de tener totalmente definido el conjunto de símbolos no terminales de la gramática.
...
\subsection{\textttb{``rules''} en \spirit}
...
El mismo estará delimitado por las cadenas ``\textttb{compute}'' y ``\textttb{end}'', en su interior se aceptará una lista con \textbf{al menos una} ecuación.
...
Cada ecuación tiene una instancia de atributo, como ``\textttb{l-value}'', el carácter ``\textttb{=}'' y una expresión como ``\textttb{r-value}''.
...
Dentro de las expresiones se tenía que ir resolviendo la precedencia de los operadores declarados por el usuario. Además, la utilización de paréntesis crea distintos niveles de precedencia. Cada vez que se utiliza la producción que genera \textttb{( E )}, la precedencia de los operadores que se utilicen en la expresión \textttb{E}, se resolverán en ese nivel.
...
Al completar esta regla se creará una \textttb{Expr\_function} dentro de la gramática.
...
\begin{center}\textbf{\large{\textttb{longest\_d\ [\ real\_p\ |\ int\_p\ ]}}}\end{center}

Utilizando la directiva \textttb{longest\_d} sobre la unión de ambos parsers predefinidos para dichos valores numéricos.

Para los demás literales se usaron las reglas definidas en un comienzo de la gramática.

Luego de parsear cada uno de los literales se crean los respectivos objetos dentro del ambiente de tipo \textttb{Expr\_literal}.
...
Una instancia debe ser un bloque que contenga una ocurrencia de un símbolo y un atributo del mismo. Por lo que no se aceptan espacios intermedios. Se utilizó la directiva \textttb{lexeme\_d}.

Con los datos obtenidos se crea un objeto de tipo \textttb{Expr\_instance}.
...
A excepción de la regla que se utilizó para definir los identificadores válidos para un \textttb{sort}, a la que se le agregaron los tipos básicos soportados por \maggen.
...
La invocación a la funcionalidad de \spirit\ se realiza con una instancia de \textttb{attritute\_grammar}, como gramática objetivo, y otra de \textttb{skip\_parser}, como parser de omisión.
...
Primero, para aceptar archivos como entrada de la herramienta, se tiene que hacer que \spirit\ no utilice un iterador estándar, sino que un \textttb{file\_iterator} instanciado con caracteres. Luego definir un \textttb{position\_iterator} sobre el recién mencionado.
...
La heurística utilizada, consiste en recorrer el \textttb{map} de símbolos no terminales y verificar que existe al menos una regla que lo tenga como parte izquierda.
...
La heurística aplicada fue recorrer el \textttb{map} de reglas, donde para cada una se verificaba que estuviesen definidas dentro de su \textttb{map} de ecuaciones, una para cada uno de sus atributos sintetizados. Y a su vez, una ecuación para cada atributo heredado de sus símbolos de la parte derecha.
...
\footnotesize{\textttb{\begin{items}
...
Teóricamente un plan es una secuencia de instancias, que indica un orden de evaluación total y consistentes de las mismas. Dentro de \maggen, se optó para su representación una secuencia de identificadores de ecuaciones. Ya que cada ecuación tiene solamente \textbf{un} \textttb{l-value}, que es precisamente una instancia. Se utiliza la clase \textttb{vector} de la STL, para su almacenamiento y están enumeradas de forma tal que pueden ser identificadas de manera univoca.

Como en general, se iban a generar en muchos casos los mismos planes, se decidió mantener un \textttb{vector} con todos los planes diferentes, así solo se generarían la \textbf{mínima cantidad de planes}, es decir, únicos necesarios.

Como en el algoritmo que se presenta en el artículo de Wuu Yang (\cite{wuu-yang1}), a cada plan se lo asocia con un \textttb{key}, primero se implementó esa clave como una estructura con los siguientes datos:
\begin{itemize}
\item La regla a la cual pertenece el plan
\item Una secuencia para computar sus atributos
\end{itemize}

Esa secuencia se representó de igual manera que un plan, ya que computar un atributo es equivalente a saber el identificador de la ecuación que lo tiene como \textttb{l-value}.
...
Al igual que con los planes de evaluación, los planes proyectados también sufrían de muchas repeticiones, por lo cual se usaron índices indirectos a un repositorio de los mismos almacenados en un \textttb{vector}.
...
Los dos primeros valores, corresponden a un \textttb{key} de un plan de evaluación, así que se usó directamente esa estructura.
...
Se mantienen dentro de la herramienta un \textttb{map} para cada uno de los tipos de grafos necesarios y además para los subgrafos ADP cíclicos que eventualmente se detecten y para optimizar la construcción de los otros, los grafos que contienen todas las instancias de cada regla sin aristas entre si.
...
La detección de ciclos, se ejectua cuando el algoritmo de visite, invoca a la función \textttb{back\_edge}, en el cual es activada la bandera que indice este suceso.
...
Al igual que con los planes, los contextos de las reglas se repiten mucho, así que se utilizaron índices indirectos sobre un \textttb{vector} que almacena únicamente los contextos diferentes.
...
Para la función \textttb{compute\_order} se debe considerar el orden de evaluación de los atributos establecido por el \textttb{index\_order}, el cual es el índice de un plan proyectado. Por lo que, primero se crea un grafo ADP copiando el grafo referenciado por \textttb{graph\_adp}, luego se le insertan los vértices necesarios, utilizando la función \textttb{return\_vertex}\footnote{Función que dado un atributo retorna un \textttb{vertex\_descriptor} de un vértice existente dentro del grafo o sino lo inserta y retorna un nuevo descriptor.} y se insertan las aristas entre los atributos consecutivos.
...
Luego se le aplica la función \textttb{warshall\_transitive\_closure}, que computa la clausura transitiva del grafo, perteneciente a la \textbf{BGL} de \boost. Esto causa que ese orden inducido por los atributos repercuta en todos los componentes del grafo.

Por último, mediante la función \textttb{generates\_topological\_order}, que como su nombre lo indica calcula el orden topológico del grafo, se obtiene el orden de evaluación consistente de las ecuaciones de la regla actual.

La función \textttb{purge\_plan\_with}, dado un plan de evaluación de ecuaciones, para una cierta regla, y otra regla, devuelve un nuevo plan, conteniendo las ecuaciones de ese plan que pertenecen a la otra regla pasada como parámetro.
...
\begin{itemize}
\item \textttb{Visit}: visitar a un nodo hijo debido a que necesita que le compute algún atributo sintetizado.
\item \textttb{Leave}: vuelve a su ancestro (padre), debido a que necesita que le calculen algún atributo heredado.
\item \textttb{Compute}: se encuentra en condiciones de computar uno de sus atributos sintetizados o heredado de sus hijos.
\end{itemize}
...
El algoritmo principal, \textttb{generate\_visit\_sequences}, es más bien un invocador de la función \textttb{gen\_visit\_seq}, que es la que realmente lleva a cabo las llamadas que simulan la evaluación.
...
Dentro de \textttb{gen\_visit\_seq}, se recorre el plan de evaluación respetando los criterios explicados en \ref{sec:constseqvisit}. Además se debe asegurar que sólo se actualice la secuencia de visita correspondiente al plan actual,si no se produjo ningún \textttb{leave}. Para ello se lleva una variable lógica (flag) que indica la ausencia de \textttb{leaves}, además una secuencia de visita y un listado de instancias de atributos computados a nivel local.

Durante su funcionamiento se cumplen las siguientes condiciones:
\begin{itemize}
\item Los atributos sintetizados del símbolo de la izquierda de la regla, son informados a nodo padre, mediante su inserción en \textttb{ins\_computed} pasado como referencia.

\item Los atributos ya sean sintetizados del símbolo de la izquierda o heredados de algún símbolo de la derecha, son insertados en \textttb{ins\_computed\_own}.

\item Siempre que se termina de recorrer un plan, se actualiza la secuencia de visita del plan actual siempre y cuando no pertenezca a \textttb{v\_seq\_computed}.

La actualización se lleva a cabo mediante la función \textttb{update\_visit\_sequence}, la cual:
\begin{items}
\item Recibe un índice que representa tanto a un plan como a su secuencia de visita y una secuencia de visita ``nueva''.

\item Empareja la vieja secuencia, verificando posición a posición que sean iguales, pero si en la secuencia ya guardada existe un \textttb{leave}, es decir, un cero, vuelve a avanzar.
...
\item Si un plan se recorrió en su totalidad, se inserta en \textttb{plans\_computed}.

\item La instancia que se encuentra en el \textttb{l-value} de la ecuación, será marcada con un \textttb{compute} solamente cuando se haya podido consumir el \textttb{r-value} de la ecuación y no se haya dado un \textttb{leave}.

\item Cuando se debe realizar un \textttb{visit}
...
\item Luego se inserta en la secuencia de visita local el \textttb{visit}.

\item Se invoca recursivamente a \textttb{gen\_visit\_seq} con:

\begin{items}
\item \textttb{ins\_computed\_child}: inicializado con todos los atributos heredados del símbolo a recursar, calculados hasta el momento.

\item \textttb{p\_computed\_child}: inicializado con todos los planes computados hasta el momento, más el índice del plan actual ``virtualmente'' ya computado para evitar ciclos de llamadas recursivas.

\item \textttb{v\_seq\_child}: inicialmente está vacío para la recursión del primer plan seleccionado, pero es acumulativo para las demás llamadas. Indica las secuencias de visita ya calculadas.
\end{items}

\item Al terminar la recursión, la \textttb{gen\_visit\_seq} (invocación), se extraen los resultados útiles para este contexto.

\begin{items}
\item Nuevos planes computados a partir de la llamada sobre el nodo hijo, exceptuando la ``computada virtual'' del plan actual. Insertados en \textttb{plans\_computed}.

\item Nuevos atributos sintetizados calculados del nodo hijo. Insertados en \textttb{ins\_computed\_own}.

\item Nuevas secuencias de visitas obtenidas completamente. Insertados en \textttb{v\_seq\_child}.
\end{items}
\end{items}
...
Para obtener un diseño dentro de los estándares de C++, se crean dos archivos: una cabecera \textttb{.hpp} y un fuente \textttb{.cpp}. Ambos archivos son creados con el nombre proporcionado por el usuario mediante la opción \textttb{maggen -o name} o sino con el nombre por defecto: \textttb{mag\_eval}\footnote{Para más detalles sobre usos, ver capítulo \ref{chap:usos}.}.

También se copian al directorio de salida de \maggen, dos archivos utilizados por el evaluador:
\begin{items}
\item \textttb{Node.hpp:} ver código fuente en apéndice \ref{append:nodehpp}
\item \textttb{Plan.hpp:} ver código fuente en apéndice \ref{append:planhpp}
\end{items}
...
La función principal es \textttb{generate\_code}, en donde se invocan a las demás funciones para generar los fuentes del evaluador.
...
Notar que recibe como parámetro una referencia a los ``\textttb{headers}'' que el usuario proporcionó en el momento de la invocación de \maggen \footnote{Ver sección \ref{sec:uso-maggen} para obtener detalles sobre parametros de invocación.}. Los mismos serán incluidos en el archivo \textttb{.cpp}.

\subsection{Cabecera del evaluador}

Dentro de este archivo, se definen para cada \textbf{símbolo no terminal} una tipo en base a una estructura que incluye a sus atributos, un constructor y un método \textttb{to\_string()} que genera una cadena con su estado actual.

\underline{Por ejemplo:} símbolo Y, del ejemplo de Wuu Yang en el apéndice \ref{append:agwuuyang}.
\begin{center}\textttb{symbol Y  NonTerminal Attributes: i2, i3, s2, s3;}\end{center}
...
Lo primero que el ``desarrollador\footnote{Un usuario que se interesa en indagar el código del evaluador.}'' se encuentra al abrir este archivo, es la implementación del constructor y del método \textttb{to\_string()} de cada uno de los símbolos no terminales. Por cuestiones del diseño realizado, cada nodo del AST tiene un identificador de regla, que debe ser un \textbf{número positivo mayor que cero}.
...
\begin{items}
\item \textttb{traverse}: responsable de seleccionar el plan de evaluación para cada nodo no terminal.
\end{items}

\item [Segunda recorrida del AST] se invoca a la función
\begin{items}
\item \textttb{eval\_visiter}: es un evaluador orientado a visitas, que debe computar los valores de los atributos de los nodos.
\end{items}
\end{description}
...
\fbox{\textttb{\textbf{void traverse (struct Node *node, unsigned short order)}}}\\

Esta función a partir del identificador de regla que posee cada nodo AST, construye un \textttb{Order\_rule}, es decir, su contexto de invocación. Luego, mediante la función \textttb{return\_index\_context} se obtiene su índice relativo dentro del arreglo que contiene a todos los contextos únicos computados por \maggen. Si no lo encuentra, produce un error ya que el AST de entrada posee un contexto incorrecto para la gramática para la cual se generó el evaluador.

Después se arma un \textttb{Key\_plan} con dicho índice y el parámetro \textttb{order}, que es el índice del plan proyectado que se corresponde con el orden de evaluación de sus ecuaciones, impuesto por su contexto superior.

Por último se busca el plan de evaluación correspondiente al nodo visitado y se lo guarda en la estructura del nodo en el campo \textttb{visit\_seq\_index}.
...
\begin{center}\textttb{Key\_plan\_project k\_plan\_proj(k\_plan, rule[i+1], i)}\end{center}

donde la variable ``\textttb{i}'' se desplazará desde 1 a la cantidad de hijos\footnote{La cantidad de hijos es idéntica a la longitud de la regla que representa al nodo.}.\\

Con lo cual se realiza la llamada recursiva de la función:

\begin{center}\textttb{traverse(node->childs[i], eval\_plans\_project[j].second);}\end{center}

donde ``\textttb{j}'' corresponde al índice del plan proyectado del hijo a visitar.
\subsubsection{Función Eval}
\fbox{\textttb{\textbf{void eval\_visiter(struct Node *root)}}}\\
...
Notar que cuando se debe realizar un \textttb{visit}, sólo consiste en llamar a el evaluador sobre el hijo indicado. Si en cambio, se debe hacer un \textttb{compute}, se llama a la función \textttb{compute\_eq}, que será explicada a continuación. Y cada vez que sucede un \textttb{leave}, se debe guardar en el nodo actual la próxima posición de la última visitada, para que en la siguiente visita, el evaluador continúe desde ese punto, omitiendo al \textttb{leave}\footnote{Con el guardado del estado de cada secuencia de visita para cada nodo, se pudo omitir el índice $i$, que se presentó en las operaciones en la sección \ref{sec:sec-visit} del capítulo introductorio.}.
...
Cuando el algoritmo \textttb{eval\_visiter} invoca a que se compute una ecuación, el responsable de llamar al método correcto es:

\begin{center}\textttb{compute\_eq(int num\_eq, struct Node *root)}\end{center}

Que en resumidas palabras, es una gran sentencia de selección múltiple (\textttb{switch}) en base al identificador de la ecuación, que determina unívocamente la función a invocar.
