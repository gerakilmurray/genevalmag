...
{
bookmarksopen=true,
pdftitle="Artículo CACIC: magGen",
...
\title{{\Large{\textbf{\rmfamily{\sc Generador de Evaluadores Estáticos para Gramática de Atributos Multiplanes\\ [1cm]  \maggen}}}}}
...
Universidad Nacional de Río Cuarto - \urllink{http://www.unrc.edu.ar/}\\
...
En 1998, Wuu Yang caracteriza una nueva familia de GA, denominada \textbf{Gramática de Atributos Multiplanes} (MAG), que permiten una mayor expresividad, presentando un algoritmo para su evaluación, basado en secuencias de visita, en tiempo polinomial.%35
...
\maggen\ es una herramienta que genera, de manera automática, evalua-dores estáticos para la familia MAG, basada en la teoría presentada por Wuu Yang. La salida de \maggen\ es un conjunto de módulos C++ optimizados para su directa utilización o integración con otras herramientas.%40
...
%\label{sec:met_eval}
Un evaluador de gramáticas de atributos debe %tener en cuenta todas las dependencias entre las instancias de atributos para seguir un orden consistente de evaluación.% de las mismas.
...
%En 1980, Uwe Kastens\cite{kastens} caracterizó las ``\textit{Gramáticas de Atributos Ordenadas}'' (OAG) y propuso un método para su evaluación, denomi%nado secuencias de visita. Estas son, secuencias de operaciones que conducen el recorrido del AST, para realizar la evaluación de las instancias de lo%s atributos. Kastens propone un método para generar las secuencias de visita en tiempo polinomial para la familia OAG.
...
%Esta familia, es presentada en la sección \ref{def:MAG} y son en las que se basa el funcionamiento de \maggen.
...
%Cada secuencia de visita finaliza con una operación \emph{leave}\footnote{La mayoría de las veces implícita, es decir este \emph{leave} esta dado por el fin de la secuencia.}.
...
%En la sección \ref{seubsec:seq-visit} se analiza la construcción de secuencias de visita para \maggen.
...
\section{\maggen}
\vspace{-0.2cm}
El desarrollo de \maggen\ se puede dividir en 4 etapas: %consideradas principales, que marcaron el proceso de desarrollo de la herramienta:
\begin{itemize}
...
\begin{figure}
    ...
    \subfigure[Tiempos de compilación]{
        ...
        \begin{tabular}{|l||c|c|}
            ...
            \rowcolor{white}\textbtt{Aritmética} & $\sim$5.98 sec   & \textbf{$\sim$1m 5.6 sec} \\ \hline
        \end{tabular}
...
%     \item Se demostró que se generan los planes y secuencias de visita que cubren todo posible árbol válido para la MAG, optimizando código y además evitando informar circularidades absurdas a fines prácticos.
