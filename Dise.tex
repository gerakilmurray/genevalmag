...
El bloque ``\textbtt{semantic domain}'' es el encargado de la definición de elementos que serán necesarios para los bloques siguientes, el mismo esta subdividido en 3 secciones, que se corresponden con la definición de sort, operadores y funciones, cada una con su sintaxis propia. 
...
\underline{Ejemplo:} \begin{center} \fbox{\textbtt{\ sort int;\ }} \end{center}
\vspace{0.2cm}
Esta línea declara el tipo ``\textbtt{int}''.
...
La sección destinada a la declaración de \textbtt{operadores} acepta 3 tipos de operadores, los cuales, difieren en su forma de uso y cantidad de operandos. Denominados, \textit{infijo, prefijo y posfijo}. 
...
\item 
\begin{center}
\fbox{\textbtt{\ op infix (\_,right) *: int, int -> int;\ }}\end{center}
\vspace{0.2cm}
...
\vspace{0.2cm}
\begin{center}
\fbox{\textbtt{\ op infix *: int, int -> int;\ }}\end{center}
\vspace{0.2cm}
donde se usan valores por defecto para precedencia y asociatividad.

\item 
\begin{center}
\fbox{\textbtt{\ op prefix (60,non\_assoc) \%: int -> int;\ }}\end{center}
\vspace{0.2cm}
...
\item 
\begin{center}
\fbox{\textbtt{\ lex: syn <string>\ of all - {T};\ }}\end{center}
\vspace{0.2cm}
...
\begin{center}
\fbox{\textbtt{\ type: inh <string>\ of all;\ }}\end{center}
\vspace{0.2cm}
...
\begin{center}
\fbox{\textbtt{\ grade: <int>\ of {E, T};\ }}\end{center}
\vspace{0.2cm}
...
\underline{Ejemplo:}\ \fbox{\ \textbtt{'literal'}\ }\\
\vspace{0.2cm}
...
Cabe aclarar que \textbtt{valor} debe ser atributo de ``T'' y ``F''.
\vspace{0.2cm}
...
\item Los índices de las instancias deben ser consistente con la cantidad de apariciones del símbolo, en la regla. Ejemplo: \\En la siguiente regla:\ \fbox{ S ::= S T }\ los índices posibles de las instancias son \textbtt{0} y \textbtt{1} para el símbolo ``S'' y \textbtt{0} para el símbolo ``T''.
...
Las figuras\footnote{\label{foot:graph} Imagen generada utilizando la herramienta \textit{graphviz} sobre el archivo \textbtt{.dot} generado por \maggen.} muestran un ejemplo de cada tipo de grafo, generadas a partir del ejemplo analizado en el apéndice \ref{append:agwuuyang}.
...
En la figura\footnote{Figura obtenida con la herramienta \textit{graphviz} sobre el archivo \textbtt{.dot} generado por \maggen.} \ref{fig:plan} se observa un plan obtenido por \maggen\ para el ejemplo del apéndice \ref{append:agwuuyang}. Este plan contiene el orden de los números de ecuación que pertenecen a la regla del plan, los planes proyectados son computados sobre el plan total, es decir con todas las dependencias.
