...
\begin{lstlisting}[escapeinside=<>, frame=shadowbox, rulesepcolor=\color{azul},language=specmag, linewidth=10cm]
...
\begin{lstlisting}[escapeinside=<>, frame=shadowbox, rulesepcolor=\color{azul}, language=specmag ]
...
assoc   = (left | right | <\texttt{\color{blue}{non-assoc}}>) 
\end{lstlisting}
\vspace{0.3cm}
...
\begin{lstlisting}[numbers=left, numberstyle=\tiny, numbersep=5pt, firstnumber=8, language=specmag, linewidth=12cm]
...
semantic domain
  sort ecuacion, lista;
...
\begin{lstlisting}[basicstyle=\scriptsize, backgroundcolor=\color{white}, language=c++]
...
Los dos módulos analizados arriba, generados por \maggen, se apoyan sobre dos módulos mas, que son anexados de manera estática por la herramienta, ellos son: \texttt{Node.hpp} y \texttt{Plan.hpp}. Los mismos contienen definiciones de ``tipos'' y ``métodos'' necesarios por el el evaluador generado.
A continuación observamos el interior de cada uno de ellos:

\lstinputlisting[basicstyle=\tiny, numbers=left, numberstyle=\tiny, numbersep=5pt, language=c++]{input_file_code/Node.hpp}

\lstinputlisting[basicstyle=\tiny, numbers=left, numberstyle=\tiny, numbersep=5pt, language=c++]{input_file_code/Plan.hpp}
